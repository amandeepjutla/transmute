...
class Example(ttk.Frame):
    ...
    def initUI(self):
        ...
        for drug in d.values():
            drug = drug.replace("'","").replace("[","").replace("]","")

            box1.insert(END, drug)
            box2.insert(END, drug)
        ...
        self.label_from = ttk.Label(self, anchor=W, textvariable=self.convert_from)
        ...
        okButton = ttk.Button(self, text="Exit")
        okButton.pack(side=BOTTOM, padx=5, pady=5)

        closeButton = ttk.Button(self, text="Convert")
